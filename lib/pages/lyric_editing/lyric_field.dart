...
class LyricEditingController extends TextEditingController {
  ...
  final _lrcModelNotifier = ValueNotifier<LrcModel>(
    LrcModel(text: '', lineTimestampRangeMapping: {}),
  );
  ...
  void _updateModel() {
    ...
    text.split('\n').forEachIndexed((index, line) {
      final regexp = RegExp(r'\[(\d{1,}):(\d{2})(?:\.(\d{1,}))?\]');
  ...
  void _updateSelectedRanges(TextSelection selection) {
  ...
  void offsetSelectedTimestamps(Duration offset) {
    ...
    final newText = text
        ...
        .mapIndexed((index, line) {
          if (!_selectedRanges.containsKey(index)) return line;

          final ranges = _selectedRanges[index]!;
          for (final TimestampRange range in ranges.reversed) {
  ...
  set value(TextEditingValue newValue) {
    ...
    if (newValue != value) _updateSelectedRanges(newValue.selection);
...
class LineHighlightController extends ChangeNotifier
  ...
  void _updateHighlightLineIndex() {
    final i = lowerBound(_timestampLineMapping.keys.toList(), _position);
    if (i == 0) {
      _setValue(null);
    } else {
  ...
  LrcModel _lrcModel = LrcModel(text: '', lineTimestampRangeMapping: {});
